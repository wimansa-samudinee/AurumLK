  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await register({
        ...
        email,
        password,
        role: userType.toUpperCase() as "CUSTOMER" | "BUSINESS",
        businessName: userType === "business" ? name : undefined,
        licenseNumber: userType === "business" ? licenseNumber : undefined,
      });
    } catch (err) {
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-border p-8">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 text-white rounded-xl font-semibold flex items-center justify-center gap-2 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all disabled:opacity-60"
              style={{ background: "linear-gradient(135deg, #f15a14 0%, #f7a414 55%, #f7c31a 100%)" }}
            >
